...
```bash
python -m pytest            # 94 tests, ~5 seconds
```
...
| `autumn_leaves.musicxml` | 3 instruments (Flute / Piano / Acoustic Bass), 8 bars of the canonical E-minor A section chord progression: `Am7 / D7 / Gmaj7 / Cmaj7 / F#m7b5 / B7 / Em / Em`. Exercises multi-instrument rendering, jazz chord analysis, walking bass digest. Melody is **original** — the real Kosma/Prévert/Mercer tune is still under copyright through the 2040s. |
| `barbershop.musicxml` | 4-voice SATB close harmony (Tenor / Lead / Baritone / Bass) over a `I / IV / V7 / I` cadence in B♭ with verse-1 lyrics on every voice. Exercises many-part layout, per-voice lyrics, 4-part chord analysis. |
| `bach_chorale.musicxml` | **Real** Bach BWV 66.6 pulled from `music21`'s bundled PD corpus (10 bars, F# minor, SATB). Exercises authentic figured-bass voice leading, pickup measures, and the Picardy-third final cadence. |
| `chopin_prelude.musicxml` | Original 8-bar E-minor piano piece in 2/2 in the style of Op.28 No.4, with dense baked-in dynamics (`p / mf / pp`) and text expressions (`dolente / cresc. / morendo`). Exercises vibe-edit persistence through save/load. |
| `polyrhythm.musicxml` | 3-against-4 cross rhythm: 4 straight quarters vs. 3 half-note triplets per bar, plus a bar of eighth-note triplets against straight quarters. Exercises tuplet handling in `score_io.fmt_duration` — digest tokens `h3` / `e3`. |
| `pop_ballad.musicxml` | Original 8-bar piano + vocal ballad in C major on the classic `I / V / vi / IV` four-chord loop. 32 hand-matched syllables, one per note. Exercises piano-plus-vocal layout, lyrics on exactly one of multiple parts, pop progression analysis. |
| `multi_voice.musicxml` | 2-staff keyboard SATB reduction: treble staff has Soprano (voice 1) + Alto (voice 2), bass staff has Tenor (voice 1) + Bass (voice 2) — all 4 voices living inside the same two `stream.Part`s via `stream.Voice` objects. Exercises the multi-voice-per-staff code path in `views.digest` and the `ops.set_note --voice 2` targeting (critical: must touch the alto, not the soprano). |
| `structure_demo.musicxml` | 12-bar AABA form with every structural marking: rehearsal marks A/B/C, repeat start, repeat end (×2), first ending `[1.]`, second ending `[2.]`, Segno, Coda, D.S., Fine, mid-piece key change (C → G), mid-piece time change (4/4 → 3/4), and a tempo jump to Allegro. Exercises `views.structure` end-to-end. |

Test modules cover:

- **views** — info/digest/structure, measure & part filtering
- **analyze** — key detection (E minor Autumn Leaves, B♭ barbershop, F# minor Bach), Roman-numeral progressions, per-beat & per-measure chord analysis
...
- **render** — magic-byte checks for PDF / MIDI / SVG / PNG / MusicXML across every fixture
- **importers** — MIDI roundtrip + transcribe graceful degradation
- **preview** — Verovio payload includes all pages + base64 MIDI
- **fixtures** — Picardy third detection on real Bach, tuplet sums for polyrhythm, I-V-vi-IV detection on pop ballad
